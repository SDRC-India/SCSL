...
function sncuDataEntryController($scope, $http, $timeout, allServices){
	$scope.activeMenu = "legacySncuDataEntry";
	$scope.pageName = "Historical Data Entry";
	$("#loader-mask").show();
	$scope.pageLoaded = false;
	$scope.tableData = [""];
	$scope.profileDataSaved = false;
	$scope.profileFormDisabled = false;
	$scope.selectionValidated = false;
...
//	if(hasLR){$scope.hasLR = "YES"}else{$scope.hasLR = "No"};
	...
	allServices.getTimePeriodForLegacy().then(function(data) {
		...
		$("#loader-mask").fadeOut();
			...
			$scope.selectDate = function(todate) {
				$scope.selectedDate = todate;

			};
			
			$scope.validateHistoricalDataSelection = function() {
				if ($scope.selectedState == undefined) {
					...
					$("#loader-mask").show();
					$scope.getFormTable(false, false);
					$scope.profileDataSaved = false;
					$scope.profileFormDisabled = false;
				}
			};
...
//	
	...
	*/
	$scope.getRestIndicators = function(){
		$http.get('getLegacyRestIndicators?timeperiodId=' +  $scope.selectedDateDataEntry.timePeriodId
				+"&facilityId="+$scope.selectedFacilityDataEntry.areaId+"&hasLr="+$scope.selectedFacilityDataEntry.hasLr).then(function(response){
	 ...
	 * 
	*/
	$scope.getFormTable = function(noAccessProfileData, showSuccessfulAddIndicator) {
			$http.get("getLegacyWebDataEntryIndicators?timeperiodId=" + ($scope.selectedDate != undefined ? $scope.selectedDate.timePeriodId : $scope.selectedDateDataEntry.timePeriodId)
			+ "&facilityId=" + ($scope.selectedFacility!=undefined ? $scope.selectedFacility.areaId : $scope.selectedFacilityDataEntry.areaId)
			+ "&hasLr=" + ($scope.selectedFacility != undefined ? $scope.selectedFacility.hasLr : $scope.selectedFacilityHasDataEntry == "Yes" ? true : false)).then(function(response){
			$("#loader-mask").fadeOut();
			if($scope.selectedFacility!=undefined){
				$scope.selectedDateDataEntry = $scope.selectedDate;
				$scope.selectedFacilityDataEntry = $scope.selectedFacility;
				$scope.selectedFacilityHasDataEntry = $scope.selectedFacility.hasLr == true ? "Yes" : "No";
			}
			$scope.getRestIndicators();
			$scope.selectionValidated = true;
...
//		$scope.allData.push($scope.profileTableData);
		...
		$http.post('saveLegacySNCUIndicatorData?timeperiodId=' + $scope.selectedDate.timePeriodId+"&facilityId="+$scope.selectedFacility.areaId,
				$scope.allData)
		.then(function(result){
			...
			$scope.link = "legacySncuDataEntry";
		}, function(error){
			console.log(error);
		});
	};
...
//		if(hasLR){
	...
	//						$scope.warningMsg = "Denominator value is blank of " + $scope.formTableData[formats[x]][i].indicatorName + " in " + formats[x];
							$scope.warningMsg = "Some fields are left blank in Intermediate section";
							$("#warningMessageSncuSubmit").modal("show");
							return false;
						}
...
//		}		
	...
	*/
	$scope.addIndicator = function(){
		...
		$http.post('addLegacyIndicatorMappings?timeperiodId=' + +$scope.selectedDateDataEntry.timePeriodId
			+ "&facilityId=" + $scope.selectedFacilityDataEntry.areaId, $scope.allSelectedIndicators)
			.then(function(result){
			$scope.getFormTable( $scope.selectedDateDataEntry.timePeriodId, true, true);
			$timeout(function(){
				$("[type='radio'] + label + .content").each(function(){$(this).height($(this).find(".table-responsive").height()+200)})
			},200)
			$scope.getRestIndicators();
